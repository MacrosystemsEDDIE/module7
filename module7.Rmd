...
```
### Function to creat NP model inputs
```{r}
# Create np inputs ----
...
yini <- c(
  ...
  DIN = 9) #mmolN m-3
#ok we are gonna need to convert mg/L to mmolN m-3 above
```
## Run forecast
```{r}
...
est_out = EnKF(n_en = n_en, 
           ...
           ##STOPPED EDITING HERE - only want to update maximumUptake parameter
           #need to go through EnKF functions one by one and make sure they are
           #behaving w/ Barco data + NP model and then put it all together
           decay_init = 0.005, 
           obs_cv = 0.1,
           param_cv = 0.2,
