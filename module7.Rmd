...
```
## Updates so far:  
- I borrowed heavily from EnKF functions developed for a GLEON workshop in 2019 (run by Jake Zwart) to develop this script. There's one discrepancy in the Kalman filter function that I'm not 100% confident I resolved correctly - Quinn or Tadhg, can you check this? The dimensions of the H matrix (the observation ID matrix) don't really make sense to me either in the function where it's originally constructed or as it's applied in the Kalman filter function. It ends up being a 3x2 when it's applied in the Kalman filter and I would have thought it would be either 3x1, since I've set things up to tune two states and one parameter...? Anyways I just don't understand the H matrix dimensions and could use some guidance there.    
  
- Following on that, I definitely think we'll need to acknowledge Jake. Tadhg and I chatted a bit about whether borrowing the code merited co-authorship for Jake and decided that since it was provided as part of an open workshop it probably wasn't required, but if Quinn/Cayelan thinks differently I'm definitely open to revisiting that.  
  
- Right now we are assimilating data for chl-a and DIN and tuning the maxUptake parameter based on those two states. The maxUptake parameter tuning is pretty boring (see final plot), I think because we only have monthly DIN observations and the covariance between the two is not super-strong (you can see the value of the maxUptake parameter reacts a bit to our second DIN observation in the plot, but it's not much). Honestly tuning a parameter may be more involved than we are aiming for with the module anyways.    
   
- I think the NP model is workable for our purposes if we want one of the learning objectives to be about how the response of the forecast to data assimilation depends on our relative confidence in observations vs. model structure. In playing around with the CV values of the various data streams, we can tune things pretty well such that sometimes data assimilation allows the model to capture blooms and other times it doesn't. Definitely check it out for yourself!  
  
- One of the immediate next steps would be to run this based on a NOAA forecast product rather than observed driver data, but I just wanted to check in with you all and get some quick feedback first.  
  
- I think there is a TON that could be done just with this very basic framework..here are some of the lines of inquiry that come to mind that dovetail nicely with our module topic and what we've brainstormed so far:  
  
1) How does our confidence in observations vs. our model affect the response of a forecast to assimilated data?  
2) How does the frequency of assimilated observations affect forecast output?  
3) How well does the forecast capture extreme events (algal bloom) with and without data assimilation?  
  
There's tons more that could be explored with the covariance structure but again that might be a little much...   
...
kalman_filter = function(Y, R, obs, H, n_en, cur_step){
  ...
  #a look, b/c not sure why we've specified a 3x2 matrix for H.....
...
est_out = EnKF(n_en = n_en, 
           ...
           driver_cv = c(0.1,0.1), # CV of driver data for wtemp and PAR, respectively 
...
## Generate 35-day "forecast" using 2019 data  
Switching over to 2019 b/c there are big gaps in wtemp data in fall 2018 that our current rough-cut linear interpolation method is probably not going to be that great at filling.   
Keeping 2018 above b/c there's no bloom event in 2019.   
  
### Data wrangling
```{r}
wtemp1 <- wtemp %>%
  filter(Date >= "2019-06-01" & Date <= "2019-09-10") %>%
  rename(wtemp = V1) %>%
  select(Date,wtemp)
...
din1 <- din %>%
  filter(collectDate >= "2019-06-01" & collectDate <= "2019-09-10") %>%
  rename(din = value,
         Date = collectDate) %>%
  select(Date,din) %>%
  group_by(Date) %>%
...
lake_data <- left_join(lake_data_0,din1,by = "Date") %>%
  rename(datetime = Date)
lake_data$chla[62:101] <- NA
lake_data$din[62:101] <- NA
```
### Run forecast  
I've got three versions here:  
1) same CVs as the run for 2018 above  
2) small CV for observations and initial conditions  
```{r}
n_en = 100 # how many ensembles 

#1) same as for 2018
est_out_2019 = EnKF(n_en = n_en, 
           start = '2019-06-01', # start date 
           stop = '2019-09-10', # stop date
           time_step = 'days',
           obs_file = lake_data,
           driver_file = lake_data,
           n_states_est = 2, 
...
#2) small CV for IC
est_out_2019_smallICCV = EnKF(n_en = n_en,
           start = '2019-06-01', # start date
           stop = '2019-09-10', # stop date
           time_step = 'days',
           obs_file = lake_data,
...
#3) big CV for IC
est_out_2019_bigICCV = EnKF(n_en = n_en,
           start = '2019-06-01', # start date
           stop = '2019-09-10', # stop date
           time_step = 'days',
           obs_file = lake_data,
           ...
           param_cv = 0.2,#for maxUptake
           driver_cv = c(0.1,0.1), # CV of driver data for wtemp and PAR, respectively
           init_cond_cv = c(0.4,0.4),#cv for chl-a and DIN, respectively
           state_names = c("chla","din"))

#4) small CV for obs
est_out_2019_smallObsCV = EnKF(n_en = n_en,
           start = '2019-06-01', # start date
           stop = '2019-09-10', # stop date
           time_step = 'days',
           obs_file = lake_data,
...
#5) big CV for obs
est_out_2019_bigObsCV = EnKF(n_en = n_en,
           start = '2019-06-01', # start date
           stop = '2019-09-10', # stop date
           time_step = 'days',
           obs_file = lake_data,
           ...
           n_params_obs = 0,
           maxUptake_init = 1.0,
           obs_cv = c(0.4,0.4),#cv for chl-a and DIN, respectively
           param_cv = 0.2,#for maxUptake
           driver_cv = c(0.1,0.1), # CV of driver data for wtemp and PAR, respectively
           init_cond_cv = c(0.2,0.2),#cv for chl-a and DIN, respectively
           state_names = c("chla","din"))

#6) big CV for obs and IC
est_out_2019_bigCV = EnKF(n_en = n_en,
           start = '2019-06-01', # start date
           stop = '2019-09-10', # stop date
           time_step = 'days',
           obs_file = lake_data,
...
#7) small CV for obs and IC
est_out_2019_smallCV = EnKF(n_en = n_en,
           start = '2019-06-01', # start date
           stop = '2019-09-10', # stop date
           time_step = 'days',
           obs_file = lake_data,
...
#8) big CV for drivers
est_out_2019_bigDriverCV = EnKF(n_en = n_en,
           start = '2019-06-01', # start date
           stop = '2019-09-10', # stop date
           time_step = 'days',
           obs_file = lake_data,
...
```{r}
# plotting 
plot_chla(est_out_2019) 
plot_chla(est_out_2019_smallICCV)
plot_chla(est_out_2019_bigICCV) 
plot_chla(est_out_2019_smallObsCV) 
plot_chla(est_out_2019_bigObsCV) 
plot_chla(est_out_2019_bigCV)
plot_chla(est_out_2019_smallCV) 
plot_chla(est_out_2019_bigDriverCV)

plot_din(est_out_2019) 
plot_din(est_out_2019_smallICCV)
plot_din(est_out_2019_bigICCV) 
plot_din(est_out_2019_smallObsCV) 
plot_din(est_out_2019_bigObsCV) 
plot_din(est_out_2019_bigCV)
plot_din(est_out_2019_smallCV) 
plot_din(est_out_2019_bigDriverCV)
