...
EnKF = function(n_en = 100, 
  ...
  PHYTO = obs_df[1,2], #mmolN m-3 which is the same as mg chl b/c ratio set to 1
  DIN = obs_df[1,3]) #mmolN m-3
  ...
  for(t in 2:n_step){
    for(n in 1:n_en){
      ...
      residual_matrix[1,] <- c(0.5, 15, 0.01)
      residual_matrix[2,] <- c(1, 10, 0.02)
      residual_matrix[3,] <- c(2.5, 5, 0.03)
      residual_matrix[4,] <- c(-0.3, -5, -0.03)

      sigma <- cov(residual_matrix)
      
      #make a draw from Y_star
      Y_draw = abs(rmvnorm(1, mean = Y_star, sigma = sigma))
...
chla <- read_neon_data(siteID, "Chlorophyll-a") %>%
  filter(year(Date) %in% c(2018,2019))  #to convert to mg for input into NP model
...
```{r}
wtemp1 <- wtemp %>%
  filter(Date >= "2019-06-10" & Date <= "2019-08-10") %>%
  rename(wtemp = V1) %>%
  select(Date,wtemp)
...
din1 <- din %>%
  filter(collectDate >= "2019-06-10" & collectDate <= "2019-08-10") %>%
  rename(din = value,
         Date = collectDate) %>%
  select(Date,din) %>%
  group_by(Date) %>%
...
est_out = EnKF(n_en = n_en, 
           start = '2019-06-11', # start date 
           stop = '2019-07-15', # stop date
           time_step = 'days',
           obs_file = lake_data_weekly_assim,
           driver_file = lake_data_weekly_assim,
           n_states_est = 2, 
           n_params_est = 1,
           n_params_obs = 0,
           maxUptake_init = 1.0, 
           obs_cv = c(0.2,0.1),#cv for chl-a and DIN, respectively
           param_cv = 0.05,#for maxUptake
           driver_cv = c(0.05,0.05), # CV of driver data for wtemp and PAR, respectively 
